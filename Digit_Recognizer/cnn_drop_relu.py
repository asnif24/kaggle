...
class Layer(VariableDefiner):
    def __init__(self, in_size, out_size):
        ...
        self.b = tf.Variable(tf.random_normal([1, out_size], mean=0.5, stddev=0.1))
...
class CNN(Convolution, VariableDefiner):
    ...
    def output(self, x):
        ...
        h_layer2 = self.layer_var["layer2"].output(h_layer1, tf.nn.sigmoid)
        h_drop = tf.nn.dropout(h_layer2, self.keep_prob)
        h_layer3 = self.layer_var["layer3"].output(h_drop, tf.nn.softmax)
        return h_layer3
